...
def install_reqs():
    reqs = ["aiogram", "aiohttp", "fake-useragent", "requests"]
    for r in reqs:
        try:
            __import__(r)
        except ImportError:
...
# Раскомментируй если нужны прокси
# PROXIES = {
...
async def check_username_http(username: str) -> Optional[bool]:
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url, headers=headers, proxy=PROXIES, ssl=False) as response:
        ...
        logger.warning(f"Timeout при проверке @{username}")
        return None
    except aiohttp.ClientConnectorError as e:
        logger.warning(f"Connection error при проверке @{username}: {e}")
        return None
...
class DatabaseManager:
    ...
    def setup(self):
        ...
        # ФИКС: добавляем недостающие колонки для старой БД
        try:
            self.cursor.execute("ALTER TABLE users ADD COLUMN filter_requests INTEGER DEFAULT 0")
        except:
            pass
        try:
            self.cursor.execute("ALTER TABLE users ADD COLUMN mask_requests INTEGER DEFAULT 0")
        except:
            pass
...
async def test_http_command(message: Message):
    ...
    await message.answer("🔍 Тестирую HTTP запросы к t.me...\n\nПроверяю несколько юзернеймов:")
    
    test_usernames = ["telegram", "durov", "test" + str(random.randint(1000, 9999)), "a" + str(random.randint(1000, 9999))]
    results = []
    
    for username in test_usernames:
        result = await check_username_http(username)
        if result is True:
            status = "✅ СВОБОДЕН"
        elif result is False:
            status = "❌ ЗАНЯТ"
        else:
            status = "⚠️ ОШИБКА (None)"
