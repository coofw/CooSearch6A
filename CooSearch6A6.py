...
def install_reqs():
    reqs = ["aiogram", "telethon", "requests"]
    for r in reqs:
        try:
            __import__(r)
        except ImportError:
...
class DatabaseManager:
    ...
    def get_daily_limit(self, user_id):
        if user_id in ADMIN_IDS:
            return float('inf')
        user_data = self.get_profile(user_id)[0]
        if not user_data:
            return 3
...
class EvaluatorEngine:
    def __init__(self):
        ...
        self.tg_words = ['durov', 'pavel', 'telegram', 'tgram', 'tgweb', 'premium', 'sticker', 'channel', 'bot', 'coofw', 'mised']
    ...
    def generate_telegram(self, length=5):
        ...
        if suitable:
            return random.choice(suitable)
        tg_chars = 'tgdurovpavelpremiumsticker'
        return ''.join(random.choices(tg_chars, k=length))
...
def check_tme_http(username):
    try:
        ...
        with urllib.request.urlopen(req, timeout=3) as response:
            ...
            if 'class="tgme_page_title"' in html or 'class="tgme_page_extra"' in html:
                return False
            return True
    except:
        return True

# ========== ФИКС №2: check_username_via_session возвращает None при флуде ==========
async def check_username_via_session(username):
    ...
    if not available_indices:
        return None  # ← ВСЕ СЕССИИ В БЛОКЕ
...
# ========== ФИКС №3: check_username_hybrid правильно обрабатывает None ==========
async def check_username_hybrid(username):
    global http_last_request
    
    session_result = await check_username_via_session(username)
    
    # Если есть доступная сессия (результат не None)
    if session_result is not None:
        return session_result, "reliable"
    
    # Все сессии в блоке - используем HTTP fallback
    # Кулдаун 2 секунды между HTTP запросами
    now = time.time()
...
async def premium_filters_menu(message: Message):
    user_id = message.from_user.id
    user_data = db.get_profile(user_id)[0]
    if not user_data:
    ...
    if premium_until:
        try:
            is_prem = datetime.strptime(premium_until, "%Y-%m-%d %H:%M:%S") > datetime.now()
        except:
            pass
    if not is_prem:
...
async def handle_search(message: Message, length: int):
    ...
    user_data = db.get_profile(user_id)[0]
    if not user_data:
        await message.answer("Ошибка профиля", reply_markup=get_main_keyboard())
        return
...
async def send_profile(chat_id: int, user_id: int):
    user_data, active_t, caught_t = db.get_profile(user_id)
    if not user_data:
        await bot.send_message(chat_id, "Ошибка профиля", reply_markup=get_main_keyboard())
        return
...
async def admin_sessions_status(message: Message):
    ...
    flood = len(flood_waits)
    await message.answer(f"📊 **Статус сессий**\n\nВсего: {total}\nАктивных: {total - flood}\nВ блоке: {flood}")

@dp.message(F.text == "🚫 Чёрный список")
async def admin_blacklist_menu(message: Message):
    if message.from_user.id not in ADMIN_IDS: return
    await message.answer("Управление чёрным списком маркета:", reply_markup=get_blacklist_admin_keyboard())
