...
class SafeMessageSender:
    def __init__(self, messages_per_second=1):
        self.queue = Queue()
        self.rate = 1.0 / messages_per_second
        self.running = True
    ...
    async def _worker(self):
        while self.running:
            try:
                chat_id, text, kwargs, future = await self.queue.get()
                try:
    ...
    async def send(self, chat_id, text, **kwargs):
        ...
        await self.queue.put((chat_id, text, kwargs, future))
        return await future
...
class DatabaseManager:
    ...
    def auto_migrate_sqlite_to_postgres(self):
        ...
        try:
            ...
            tables = [
                ...
                "blacklist", "crypto_invoices", "donations", "referrals", "roulette_spins"
            ]
    ...
    def _create_postgres_tables(self, cursor):
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS roulette_spins (user_id INTEGER PRIMARY KEY, last_spin_date TEXT, prize_type TEXT, prize_value INTEGER)")
    ...
    def _create_sqlite_tables(self):
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS roulette_spins (user_id INTEGER PRIMARY KEY, last_spin_date TEXT, prize_type TEXT, prize_value INTEGER)''')
    ...
    # ========== РУЛЕТКА ==========
    def can_spin_roulette(self, user_id):
        self.cursor.execute("SELECT last_spin_date FROM roulette_spins WHERE user_id = ?", (user_id,))
        row = self.cursor.fetchone()
        if not row:
            return True
        return row[0] != datetime.now().strftime("%Y-%m-%d")

    def save_roulette_spin(self, user_id, prize_type, prize_value):
        self.cursor.execute(
            "INSERT OR REPLACE INTO roulette_spins (user_id, last_spin_date, prize_type, prize_value) VALUES (?, ?, ?, ?)",
            (user_id, datetime.now().strftime("%Y-%m-%d"), prize_type, prize_value)
        )
        self.conn.commit()
...
def get_main_keyboard():
    ...
    builder.row(KeyboardButton(text="🎡 Рулетка"))
...
async def roulette_start(message: Message):
    ...
    if not db.can_spin_roulette(user_id):
        await message.answer("⏳ Вы уже крутили рулетку сегодня! Возвращайтесь завтра.", reply_markup=get_main_keyboard())
        return
    
    is_premium = is_premium_user(user_data)
    
    # Призы: (тип, значение, название, только для премиум?)
    prizes = [
        ("searches", random.randint(1, 3), "обычных поисков", False),
        ("mirror", random.randint(1, 3), "зеркальных поисков", False),
        ("premium", random.choice([10, 60, 1440]), "минут Premium", False),
    ]
    
    # Звёзды (редко, только для премиум)
    if is_premium and random.random() < 0.3:
        prizes.append(("stars", random.choice([15, 50, 100]), "звёзд", True))
    
    prize = random.choice(prizes)
    prize_type, prize_value, prize_name, _ = prize
    
    # Выдаём приз
    if prize_type == "searches":
        db.add_searches_balance(user_id, prize_value)
    elif prize_type == "mirror":
        db.add_mirror_searches(user_id, prize_value)
    elif prize_type == "premium":
        db.add_premium_time(user_id, timedelta(minutes=prize_value))
    elif prize_type == "stars":
        db.add_stars(user_id, prize_value)
    
    db.save_roulette_spin(user_id, prize_type, prize_value)
    
    msg = await message.answer("🎡 Крутим рулетку... 🎲")
    await asyncio.sleep(1.5)
    await msg.edit_text(
        f"🎉 **ВЫ ВЫИГРАЛИ!**\n\n"
        f"🏆 {prize_value} {prize_name}\n\n"
        f"Приходите завтра за новым призом! 🎁",
...
async def main():
    global bot_username, safe_sender
    temp_dispute_data.clear()
    await load_user_sessions()
    bot_username = await get_bot_username()
    safe_sender = SafeMessageSender(messages_per_second=1)
