...
def install_reqs():
    reqs = ["aiogram", "requests", "fake-useragent"]
    for r in reqs:
        try:
            __import__(r)
        except ImportError:
...
def is_username_free(username: str) -> Optional[bool]:
    ...
    try:
        ...
        for kw in ['this username is on sale', 'fragment.com', 'buy this username', 'place a bid', 'ton']:
            if kw in html:
                return False
        
        # Реальный пользователь (НЕ ПОКАЗЫВАЕМ)
        for kw in ['tgme_page_title', 'tgme_page_extra', 'last seen', 'subscribers']:
            if kw in html:
                return False
        
        # Удалённый или замороженный (ПОКАЗЫВАЕМ)
        for kw in ['sorry, this username is already taken', 'this username is not available', 'account deleted']:
            if kw in html:
                return True
...
async def check_username_available(username: str) -> Optional[bool]:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, is_username_free, username)
...
class DatabaseManager:
    ...
    def setup(self):
        ...
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_queue_user ON search_queue(user_id, status)")
    ...
    def add_to_queue(self, user_id, query_type, length, filter_type, use_digits, count=1):
        priority = self.get_user_priority(user_id)
        self.cursor.execute("""
            INSERT INTO search_queue (user_id, query_type, length, filter_type, use_digits, count, priority, created_at, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')
        """, (user_id, query_type, length, filter_type, 1 if use_digits else 0, count, priority, datetime.now().isoformat()))
        self.conn.commit()
        return self.cursor.lastrowid
    ...
    def get_next_queue_item(self):
        self.cursor.execute("""
            SELECT id, user_id, query_type, length, filter_type, use_digits, count, priority
            FROM search_queue 
            WHERE status = 'pending'
        ...
        if row:
            return {
                'id': row[0], 'user_id': row[1], 'query_type': row[2],
                'length': row[3], 'filter_type': row[4], 'use_digits': bool(row[5]),
                'count': row[6], 'priority': row[7]
            }
        return None
    ...
    def get_user_queue_items(self, user_id, limit=10):
        self.cursor.execute("""
            SELECT id, query_type, count, status, created_at, completed_at, results
            FROM search_queue 
            WHERE user_id = ?
    ...
    def get_queue_items(self, limit=20):
        self.cursor.execute("""
            SELECT id, user_id, query_type, count, status, priority, created_at
            FROM search_queue
...
async def perform_single_search(length: int, filter_type: str, use_digits: bool, user_id: int) -> Optional[Dict]:
    while True:
        ...
        is_free = await check_username_available(username)
        if is_free is None:
            return None
        if is_free:
...
async def queue_worker(bot_instance: Bot):
    ...
    logger.warning("Очередь-воркер запущен")
    ...
    while queue_worker_running:
        try:
            ...
            try:
                await bot_instance.send_message(item['user_id'], f"🔄 Начинаю обработку...")
            except:
                pass
            
            if item['query_type'] == 'single':
                result = await perform_single_search(
                    item['length'], item['filter_type'], 
                    item['use_digits'], item['user_id']
                )
                if result:
                    db.add_search(item['user_id'])
                    text = f"✅ **Найдено:**\n\n┌ `@{result['username']}`\n├ {result['length']} букв\n├ ⭐️ {result['score']}/10 ({result['verdict']})\n└ 🟢 Свободен\n\n🔗 https://t.me/{result['username']}"
                    await bot_instance.send_message(item['user_id'], text, disable_web_page_preview=True)
                else:
                    await bot_instance.send_message(item['user_id'], "❌ Ошибка. Попробуйте позже.")
                
                db.complete_queue_item(item['id'], [result] if result else [])
                
            elif item['query_type'] == 'mass':
                remaining_searches = db.get_remaining_searches(item['user_id'])
                max_allowed = min(item['count'], remaining_searches)
                
                if max_allowed <= 0:
                    await bot_instance.send_message(item['user_id'], f"❌ Нет доступных поисков! Лимит: {db.get_user_limit(item['user_id'])}/день")
                    db.complete_queue_item(item['id'], [])
                    continue
                
                if max_allowed < item['count']:
                    await bot_instance.send_message(item['user_id'], f"⚠️ Осталось {max_allowed} поисков из {item['count']}. Будет выполнено {max_allowed}.")
                
                results = []
                progress_msg = await bot_instance.send_message(item['user_id'], f"📦 Массовый поиск: 0/{max_allowed}...")
                ...
                for i in range(max_allowed):
                    ...
                    if (i + 1) % 3 == 0 or i == max_allowed - 1:
                        try:
                            await progress_msg.edit_text(f"📦 Массовый поиск: {i+1}/{max_allowed}...")
                        except:
                            pass
                ...
                if results:
                    ...
                    with open(txt_filename, 'rb') as f:
                        await bot_instance.send_document(item['user_id'], BufferedInputFile(f.read(), filename=txt_filename), caption=f"📊 Найдено: {len(results)}")
                    ...
                    await bot_instance.send_message(item['user_id'], "❌ Ничего не найдено.")
...
async def search_5(message: Message):
    await add_to_queue(message, 5, 'single')

@dp.message(F.text == "🔎 Поиск (6 букв)")
async def search_6(message: Message):
    await add_to_queue(message, 6, 'single')

async def add_to_queue(message: Message, length: int, query_type: str):
    user_id = message.from_user.id
    
    remaining = db.get_remaining_searches(user_id)
    if remaining <= 0 and user_id not in ADMIN_IDS:
        await message.answer(f"❌ Лимит {db.get_user_limit(user_id)} поисков/день исчерпан. Купи Premium!", reply_markup=get_main_keyboard())
        return
    ...
    db.add_to_queue(user_id, query_type, length, filter_type, use_digits, 1)
    ...
    await message.answer(
        ...
        f"📅 Осталось: {remaining - 1}/{db.get_user_limit(user_id)}\n\n"
...
async def cancel_queue_start(message: Message, state: FSMContext):
    ...
    for item in items:
        if item[3] == 'pending':
            text += f"🆔 #{item[0]} | {item[1]} x{item[2]} | {item[3]}\n"
...
async def mass_search_count_input(message: Message, state: FSMContext):
    try:
        ...
        remaining = db.get_remaining_searches(user_id)
        if remaining <= 0 and user_id not in ADMIN_IDS:
            await message.answer(f"❌ Лимит {db.get_user_limit(user_id)} поисков/день исчерпан.", reply_markup=get_main_keyboard())
            await state.clear()
            return
        
        filter_type, use_digits = db.get_user_filter(user_id)
        
        db.add_to_queue(user_id, 'mass', 5, filter_type, use_digits, count)
        position = db.get_queue_position(user_id)
...
async def admin_queue_stats(message: Message):
    ...
    if items:
        ...
        for item in items:
            priority_emoji = "🟢" if item[5] == 2 else "🟣" if item[5] == 1 else "🔵"
            text += f"{priority_emoji} #{item[0]} | user {item[1]} | {item[2]} x{item[3]} | {item[4]}\n"
...
# ========== РУЛЕТКА (ФИКС: УБРАН edit_text С reply_markup) ==========
...
async def roulette(message: Message):
    ...
    # ФИКС: удаляем старое сообщение и отправляем новое с клавиатурой
    await msg.delete()
    await message.answer(
...
async def queue_stats_callback(call: CallbackQuery):
    ...
    if items:
        ...
        for item in items[:5]:
            priority_emoji = "🟢" if item[5] == 2 else "🟣" if item[5] == 1 else "🔵"
            text += f"{priority_emoji} {item[2]} x{item[3]}\n"
    ...
    while True:
        try:
            ...
            for t_user_id, t_username in active_traps:
                is_free = await check_username_available(t_username)
                if is_free is True:
...
async def main():
    ...
    logger.warning("✅ Бот запущен с WAL и фиксами!")
